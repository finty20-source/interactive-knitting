...
armhole_extra_cm = st.number_input("Доп.ширина к пройме (см, для оверсайз)", min_value=0, value=6)
...
armhole_extra_stitches = int((armhole_extra_cm / 10) * density_stitches)
...
st.write(f"🔹 Доп.ширина по пройме (оверсайз): {armhole_extra_cm} см → {armhole_extra_stitches} п.")
...
armhole_increases = distribute_increases(stitches_chest, stitches_chest + armhole_extra_stitches, rows_armhole)
